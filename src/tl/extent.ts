...
import {Coordinate, Coordinates, FlatCoordinates, Rings} from "./coordinate";
 ...
 * @api
 */
export function boundingExtent(coordinates: Array<Coordinate>): Extent {
  const extent: Extent = createEmpty();
  for (let i: number = 0, ii: number = coordinates.length; i < ii; ++i) {
 ...
 */
function _boundingExtentXYs(xs: Array<number>, ys: Array<number>, dest: Extent): Extent {
 ...
 * @return {import("./extent/Relationship").default} The relationship (bitwise compare with
 *     import("./extent/Relationship").Relationship).
 */
export function coordinateRelationship(extent: Extent, coordinate: Coordinate): number {
  ...
  let relationship: number = Relationship.UNKNOWN;
  if (x < minX) {
 ...
 * @return {Extent} Extent.
 */
export function extendCoordinates(extent: Extent, coordinates: Coordinates): Extent {
 ...
 * @return {Extent} Extent.
 */
export function extendRings(extent: Extent, rings: Rings): Extent {
  for (let i: number = 0, ii: number = rings.length; i < ii; ++i) {
 ...
 * @param {Extent} extent Extent.
 * @param {function(import("./coordinate").Coordinate): S} callback Callback.
 * @return {S|boolean} Value.
 * @template S
 */
export function forEachCorner(extent: Extent, callback: (coordinate: Coordinate) => any) {
  let val;
 ...
 * @param {Extent} extent Extent.
 * @return {import("./coordinate").Coordinate} Bottom left coordinate.
 * @api
 */
 ...
 * @param {Extent} extent Extent.
 * @return {import("./coordinate").Coordinate} Bottom right coordinate.
 * @api
 */
 ...
 * @return {import("./coordinate").Coordinate} Corner coordinate.
 */
 ...
 * @return {Extent} Extent.
 */
export function getForViewAndSize(center: Coordinate, resolution: number, rotation: number, size: Size, dest?: Extent): Extent {
 ...
 * @param {Extent} extent The extent.
 * @return {import("./size").Size} The extent size.
 * @api
 */
 ...
 * @param {Extent} extent Extent.
 * @return {import("./coordinate").Coordinate} Top left coordinate.
 * @api
 */
 ...
 * @param {Extent} extent Extent.
 * @return {import("./coordinate").Coordinate} Top right coordinate.
 * @api
 */
export function getTopRight(extent: Extent): Size {
 ...
 * @param {Extent} extent The extent.
 * @param {import("./coordinate").Coordinate} start Segment start coordinate.
 * @param {import("./coordinate").Coordinate} end Segment end coordinate.
