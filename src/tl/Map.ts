...
import {DEVICE_PIXEL_RATIO, PASSIVE_EVENT_LISTENERS} from './has';
...
export interface FrameState {
  ...
  mapId: string;
  renderTargets: {[key: string]: boolean};
}

type PostRenderFunction = (map: Map, frameState?: FrameState) => any;

interface AtPixelOptions {
  layerFilter?: undefined | (Layer<Source>) => boolean;
  hitTolerance?: number;
 ...
 * @property {Collection<import("./contrtl/Control").default>} [controls] Controls.
 ...
 * @property {Collection<import("./contrtl/Control").default>|Array<import("./contrtl/Control").default>} [controls]
 * Controls initially added to the map. If not specified,
 * {@link module:tl/contrtl/defaults.defaults} is used.
 ...
 * {@link module:tl/interaction/defaults.defaults} is used.
 ...
 * time, {@link module:tl/Map~Map#setTarget} must be called for the map to be
 ...
 * {@link module:tl/Map~Map#setView}.
 */
 ...
 *
 *     import Map from 'tl/Map';
 *     import View from 'tl/View';
 *     import TileLayer from 'tl/layer/Tile';
 *     import OSM from 'tl/source/OSM';
 *
 *     const map = new Map({
 *       view: new View({
 *         center: [0, 0],
 ...
 *     });
 *
 * The above snippet creates a map using a {@link module:tl/layer/Tile~TileLayer} to
 * display {@link module:tl/source/OSM~OSM} OSM data and render it to a DOM
 * element with the id `map`.
 *
 * The constructor places a viewport container (with CSS class name
 * `tl-viewport`) in the target element (see `getViewport()`), and then two
 * further elements within the viewport: one with CSS class name
 * `tl-overlaycontainer-stopevent` for controls and some overlays, and one with
 * CSS class name `tl-overlaycontainer` for other overlays (see the `stopEvent`
 * option of {@link module:tl/Overlay~Overlay} for the difference). The map
 * itself is placed in a further element within the viewport.
 *
 * Layers are stored as a {@link module:tl/Collection~Collection} in
 * layerGroups. A top-level group is provided by the library. This is what is
 * accessed by `getLayerGroup` and `setLayerGroup`. Layers entered in the
 * options are added to this group, and `addLayer` and `removeLayer` change the
 * layer collection in the group. `getLayers` is a convenience function for
 * `getLayerGroup().getLayers()`. Note that {@link module:tl/layer/Group~LayerGroup}
 * is a subclass of {@link module:tl/layer/Base~BaseLayer}, so layers entered in the
...
class Map extends BaseObject {
  ...
  private maxTilesLoading_: boolean;
  private pixelRatio_: number;
  private postRenderTimeoutHandle_?: ReturnType<typeof setTimeout>;
  private animationDelayKey_?: number;
  ...
  private previousExtent_: Extent;
  ...
  constructor(options) {
    ...
    this.viewport_.className =
      'tl-viewport' + ('ontouchstart' in window ? ' tl-touch' : '');
    ...
    this.overlayContainer_.className = 'tl-overlaycontainer';
    ...
    this.overlayContainerStopEvent_.className = 'tl-overlaycontainer-stopevent';
    ...
    /**
     * @type {Collection<import("./contrtl/Control").default>}
     * @protected
     */
     ...
     * @private
     * @type {Object<string, import("./Overlay").default>}
     */
    ...
    this.controls.addEventListener(
      ...
      /**
       * @param {import("./Collection").CollectionEvent<import("./contrtl/Control").default>} event CollectionEvent
       */
      (event) => {
    ...
    this.controls.forEach(
      /**
       * @param {import("./contrtl/Control").default} control Control.
       */
      (control) => {
        control.setMap(this);
      }
   ...
   * available on {@link module:tl/Collection~Collection}. This can be used to
   ...
   * {@link module:tl/Collection~Collection}.
   * @param {import("./layer/Base").default} layer Layer.
   * @api
   */
  addLayer(layer) {
   ...
   * @param {function(import("./Feature").FeatureLike, import("./layer/Layer").default<import("./source/Source").default>, import("./geom/SimpleGeometry").default): T} callback Feature callback. The callback will be
   *     called with two arguments. The first argument is one
   *     {@link module:tl/Feature~Feature feature} or
   *     {@link module:tl/render/Feature~RenderFeature render feature} at the pixel, the second is
   *     the {@link module:tl/layer/Layer~Layer layer} of the feature and will be null for
   ...
   * @api
   */
  public getEventPixel(event: UIEvent | {clientX: number, clientY: number}): Pixel {
   ...
   * @return {import("./coordinate").Coordinate} The coordinate for the pixel position.
   * @api
   */
   ...
   * @return {Collection<import("./contrtl/Control").default>} Controls.
   * @api
   */
  getControls() {
   ...
   * @return {import("./Overlay").default} Overlay.
   * @api
   */
  getOverlayById(id) {
   ...
   * @param {import("./coordinate").Coordinate} coordinate A map coordinate.
   ...
   * @return {import("./size").Size|undefined} The size in pixels of the map in the DOM.
   * @observable
   * @api
   */
  getSize() {
    return /** @type {import("./size").Size|undefined} */ (
   ...
   * so clicks and gestures on an overlay will trigger {@link module:tl/MapBrowserEvent~MapBrowserEvent}
   * events.
   ...
   * don't trigger any {@link module:tl/MapBrowserEvent~MapBrowserEvent}.
   ...
   * @param {import("./contrtl/Control").default} control Control.
   * @return {import("./contrtl/Control").default|undefined} The removed control (or undefined
   ...
   * @param {import("./size").Size|undefined} size The size in pixels of the map in the DOM.
   * @observable
   * @api
   */
  setSize(size) {
  ...
  if (options.controls !== undefined) {
    if (Array.isArray(options.controls)) {
      ...
      assert(
        ...
        47
      ); // Expected `controls` to be an array or an `import("./Collection").Collection`
    ...
    if (Array.isArray(options.interactions)) {
      ...
      assert(
        typeof (/** @type {?} */ (options.interactions).getArray) ===
          'function',
        48
      ); // Expected `interactions` to be an array or an `import("./Collection").Collection`
  ...
  if (options.overlays !== undefined) {
    if (Array.isArray(options.overlays)) {
      ...
      assert(
        ...
        49
      ); // Expected `overlays` to be an array or an `import("./Collection").Collection`
