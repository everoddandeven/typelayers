...
import {getInteriorPointsOfMultiArray} from './flat/interiorpoint';
import {inflateMultiCoordinatesArray} from './flat/inflate';
import {intersectsLinearRingMultiArray} from './flat/intersectsextent';
import {
...
class MultiPolygon extends SimpleGeometry {
  /**
   * @param {Array<Array<Array<import("../coordinate").Coordinate>>|Polygon>|Array<number>} coordinates Coordinates.
   *     For internal use, flat coordinates in combination with `layout` and `endss` are also accepted.
   * @param {import("./Geometry").GeometryLayout} [layout] Layout.
   * @param {Array<Array<number>>} [endss] Array of ends for internal use with flat coordinates.
   */

  private endss_: FlatCoordinates[];
  private flatInteriorPointsRevision_: number;
   ...
   * @api
   */
  public appendPolygon(polygon: Polygon): void {
    /** @type {Array<number>} */
    let ends: FlatCoordinates;
  ...
  public clone(): MultiPolygon {
    const len: number = this.endss_.length;
    const newEndss: FlatCoordinates[] = new Array(len);
    for (let i: number = 0; i < len; ++i) {
      newEndss[i] = this.endss_[i].slice();
    }

    const multiPolygon: MultiPolygon = new MultiPolygon(
   ...
   *     constructed.
   * @return {Array<Array<Array<import("../coordinate").Coordinate>>>} Coordinates.
   * @api
   */
  public getCoordinates(right: boolean): Coordinate[][][] {
   ...
   */
  public getEndss(): FlatCoordinates[] {
    return this.endss_;
  }
  ...
  public getFlatInteriorPoints(){
    if (this.flatInteriorPointsRevision_ != this.getRevision()) {
      const flatCenters: number[] = linearRingssCenter(
        this.flatCoordinates,
        0,
        this.endss_,
        this.stride
  ...
  /**
   * Return the interior points as {@link module:tl/geom/MultiPoint~MultiPoint multipoint}.
   ...
   * @api
   */
  public getPolygon(index: number): Polygon {
  ...
  public getPolygons(): Polygon[] {
    ...
    const endss: FlatCoordinates[] = this.endss_;
    const polygons: Polygon[] = [];
    let offset: number = 0;
    for (let i: number = 0, ii: number = endss.length; i < ii; ++i) {
      const ends: FlatCoordinates = endss[i].slice();
      const end: number = ends[ends.length - 1];
      if (offset !== 0) {
        for (let j: number = 0, jj: number = ends.length; j < jj; ++j) {
          ends[j] -= offset;
        }
      }
   ...
   * @param {!Array<Array<Array<import("../coordinate").Coordinate>>>} coordinates Coordinates.
   * @param {import("./Geometry").GeometryLayout} [layout] Layout.
   * @api
   */
  public setCoordinates(coordinates: Coordinate[][][], layout: GeometryLayout): void {
