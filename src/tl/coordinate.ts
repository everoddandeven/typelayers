...
export type Rings = Coordinates[];
export type FlatCoordinates = Array<number>;
export type RingCoordinates = number[][];
export type CoordinateFormat = (coordinate?: Coordinate) => string;
export type XYCoordinate = [number, number];
export type XYZCoordinate = [number, number, number];
export type XYMCoordinate = [number, number, number];
export type XYZMCoordinate = [number, number, number, number];
//export type GeometryLayout = 'XY' | 'XYZ' | 'XYM' | 'XYZM';
//export default class FlatCoordinates extends Array<number>
//{
//  slice(start?: number, end?: number): FlatCoordinates {
//    return <FlatCoordinates>super.slice(start, end);
//  }
//}
 ...
 * @param {import("./geom/Circle").default} circle The circle.
...
export function closestOnCircle(coordinate: Coordinate, circle: Circle): Coordinate {
  ...
  const x0 = center[0];
  const y0 = center[1];
  const x1: number = coordinate[0];
  const y1: number = coordinate[1];
 ...
 */
export function closestOnSegment(coordinate: Coordinate, segment: Coordinates): Coordinate {
  const x0: number = coordinate[0];
  const y0: number = coordinate[1];
  const start: Coordinate = segment[0];
  const end: Coordinate = segment[1];
...
/**
 * Returns a {@link module:tl/coordinate~CoordinateFormat} function that can be
 * used to format
 * a {Coordinate} to a string.
 *
 * Example without specifying the fractional digits:
 *
 *     import {createStringXY} from 'tl/coordinate';
 *
 *     const coord = [7.85, 47.983333];
...
/**
 * Transforms the given {@link module:tl/coordinate~Coordinate} to a string
 * using the given string template. The strings `{x}` and `{y}` in the template
 * will be replaced with the first and second coordinate values respectively.
 *
 * Example without specifying the fractional digits:
 *
 *     import {format} from 'tl/coordinate';
 *
 *     const coord = [7.85, 47.983333];
...
export function equals(coordinate1: Coordinate, coordinate2: Coordinate): boolean {
  ...
  for (let i: number = coordinate1.length - 1; i >= 0; --i) {
 ...
 * Example:
 *
 *     import {rotate} from 'tl/coordinate';
 *
 *     const coord = [7.85, 47.983333];
...
export function rotate(coordinate: Coordinate, angle: number): Coordinate {
  ...
  const x: number = coordinate[0] * cosAngle - coordinate[1] * sinAngle;
  const y: number = coordinate[1] * cosAngle + coordinate[0] * sinAngle;
  coordinate[0] = x;
  coordinate[1] = y;
  return coordinate;
}
 ...
 * Example:
 *
 *     import {scale as scaleCoordinate} from 'tl/coordinate';
 *
 *     const coord = [7.85, 47.983333];
 *     const scale = 1.2;
...
export function squaredDistance(coord1: Coordinate, coord2: Coordinate): number {
  const dx: number = coord1[0] - coord2[0];
  const dy: number = coord1[1] - coord2[1];
  return dx * dx + dy * dy;
}
 ...
 */
export function squaredDistanceToSegment(coordinate: Coordinate, segment: Coordinates): number {
 ...
 *
 *     import {toStringHDMS} from 'tl/coordinate';
 *
 *     const coord = [7.85, 47.983333];
 ...
 * Example without specifying fractional digits:
 *
 *     import {toStringXY} from 'tl/coordinate';
 *
 *     const coord = [7.85, 47.983333];
