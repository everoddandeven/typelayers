 ...
 * @api
 */
export function register(proj4): void {
  registered = proj4;
  ...
  for (i = 0; i < len; ++i) {
    ...
    if (!get(code)) {
      ...
      let units = /** @type {import("./Units").Units} */ (def.units);
 ...
 */
let epsgLookup = async function (code: number): Promise<string> {
 ...
 * By default, the {@link module:tl/proj/proj4.fromEPSGCode} function uses the
 ...
 * @api
 */
export function setEPSGLookup(func: (n: number) => Promise<string>): void {
  epsgLookup = func;
}
 ...
 */
export function getEPSGLookup(): (n: number) => Promise<string> {
  return epsgLookup;
}
 ...
 * call {@link module:tl/proj/proj4.register} before using this function.
 *
 ...
 * Call {@link module:tl/proj/proj4.setEPSGLookup} use the function for lookups
...
export function epsgLookupMapTiler(key) {
  return async function (code) {
    const response = await fetch(
      `https://api.maptiler.com/coordinates/search/code:${code}on?transformations=true&exports=true&key=${key}`
    );
    if (!response.ok) {
      throw new Error(
