...
export interface LayerOptions<SourceType = Source> {
  className?: string;
  opacity?: number;
  visible?: boolean;
  extent?: Extent;
 ...
 * @api
 */
class Layer<SourceType = Source, RenderType = LayerRenderer> extends BaseLayer {
   ...
   */
  public getLayersArray(array: Layer[]): Layer[] {
    array = array ? array : [];
    array.push(<Layer>this);
    return array;
  }
  ...
  public getFeatures(pixel: Pixel): Promise<FeatureLike[]> {
    ...
    return (<LayerRenderer>this.renderer_).getFeatures(pixel);
  }
  ...
  public getData(pixel: Pixel): Uint8ClampedArray | Uint8Array | Float32Array | DataView | null {
    ...
    return (<LayerRenderer>this.renderer_).getData(pixel);
  }
   ...
   */
  public getRenderer(): LayerRenderer {
    if (!this.renderer_) {
      this.renderer_ = this.createRenderer();
    }
    return <LayerRenderer>this.renderer_;
  }
  ...
  public disposeInternal() {
    if (this.renderer_) {
      (<LayerRenderer>this.renderer_).dispose();
